...
if not args.wandb_sweepid is None:
    ...
    layers = [{'num_neurons': args.hidden_size, 'activation': args.activation}] * args.num_layers
    nn = NeuralNetwork(input_dim=X_train_flattened.shape[1], output_dim=one_hot_label.shape[1], nn_archtre=layers, 
                    last_layer_activation='softmax', weight_initializer=args.weight_init)
    nn.train(train_data=train_data, val_data=val_data, epochs=args.epochs, learning_rate=args.learning_rate,
                    optimizer=args.optimizer, weight_decay=args.weight_decay, batch_size=args.batch_size, print_every_epoch=1, 
                    momentum_beta=args.momentum, beta=args.beta, beta1=args.beta1, beta2=args.beta2, epsilon=args.epsilon, loss=args.loss)
